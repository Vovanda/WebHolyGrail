...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`site_settings_theme_available_themes\` (
  	\`order\` integer NOT NULL,
  	\`parent_id\` integer NOT NULL,
  	\`value\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `);
  await db.run(
    sql`CREATE INDEX \`site_settings_theme_available_themes_order_idx\` ON \`site_settings_theme_available_themes\` (\`order\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`site_settings_theme_available_themes_parent_idx\` ON \`site_settings_theme_available_themes\` (\`parent_id\`);`,
  );
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`site_settings_theme_available_themes\`;`);
