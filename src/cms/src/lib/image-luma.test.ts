...
const plain = (r: number, g: number, b: number) =>
...
describe('тёмная ли картинка', () => {
  it('чёрная - тёмная', async () => {
    expect(await isDarkImage(await plain(0, 0, 0))).toBe(true);
  });

  it('белая - светлая', async () => {
    expect(await isDarkImage(await plain(255, 255, 255))).toBe(false);
  });

  it('насыщенный синий - тёмный: глаз видит его темнее зелёного', async () => {
    expect(await isDarkImage(await plain(0, 0, 255))).toBe(true);
  });

  it('светло-зелёный - светлый', async () => {
    expect(await isDarkImage(await plain(150, 240, 150))).toBe(false);
  });
