 ...
 * запускается сам, из фоновой задачи. Кодирование ролика занимает минуты —
...
export async function buildHls({
  ...
  // Адрес раздачи случайный: по номеру медиафайла путь к сегментам собирался
  // бы перебором. Расшифровать поток это всё равно не даст, но и список
  // файлов посторонним ни к чему.
  const prefix = `hls/${randomUUID()}`;
