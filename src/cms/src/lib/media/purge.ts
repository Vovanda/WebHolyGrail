...
/**
 * Отложенная уборка удалённых роликов.
 *
 * @remarks
 * Удаление ролика — единственное необратимое действие в системе: оригинал
 * стёрт сразу после нарезки, и восстановить его неоткуда. Поэтому «удалить»
 * означает пометить: ролик пропадает с сайта немедленно, а файлы лежат ещё
 * несколько недель.
 *
...
export async function purgeDeleted({
  ...
  for (const candidate of candidates) {
    ...
    // стоят дешевле, чем стёртый ролик, который собирались вернуть.
