...
export interface Viewer {
   ...
   *
   * @remarks
   * Владелец смотрит своё всегда: иначе перед публикацией нельзя убедиться,
   * что залит нужный файл, — закрытый ролик не открылся бы даже тому, кто его
   * загрузил.
   */
  readonly ownsVideo?: boolean | undefined;
  /** Администратор: видит всё, включая чужое закрытое. */
