 ...
 *
 * @remarks
 * Секрет у каждого ролика свой, поэтому держать их в хранилище секретов нельзя:
 * на тысячу роликов пришлась бы тысяча записей и поход по сети на каждую выдачу
 ...
 * бесполезен, а ходить по сети за каждым роликом не приходится: мастер-ключ
 ...
 * Это нужно, чтобы включение ключа не обрушило уже залитые ролики и чтобы
...
/**
 * Заворачивает секрет ролика в мастер-ключ.
 *
 * @remarks
 ...
 * мастер-ключа сделало бы неиграбельными все ролики, залитые до него.
 */
export function unwrapSecret(stored: string, key: Buffer | null): Buffer {
  ...
  if (!key) {
    throw new Error('Секрет ролика завёрнут, а VIDEO_MASTER_KEY не задан — развернуть нечем.');
  }
