  ...
  handler: async ({ input, req }) => {
    ...
    try {
      ...
      const summary = await buildHls({
        ...
        mediaId,
        /*
          Конечный адрес, по которому и надо идти за ключом: путь без домена
          на дверь сайта. Полный адрес цементировал бы в файле сегодняшний
          домен - сменится он, и записи несут в себе прошлый, а сам адрес CMS
          видит всякий, кто откроет манифест.

          Дверь сайта, а не `/api`: манифест читает браузер на домене сайта,
          и туда же уходит кука идентичности. У CMS снаружи спрашивать нечего
          (R15). Пишем сразу конечным, чтобы адрес не переписывали по дороге.
        */
        keyUri: `/internal/video/key/${mediaId}`,
