...
export const Entitlements: CollectionConfig = {
  slug: 'entitlements',
  labels: { singular: 'Доступ к материалу', plural: 'Доступы к материалам' },
  admin: {
    useAsTitle: 'id',
    defaultColumns: ['viewer', 'ref', 'resource', 'source', 'expiresAt'],
    group: 'Медиа',
    description: 'Кому открыт закрытый материал. Выдаётся оплатой или решением владельца.',
  },
  fields: [
    {
      name: 'viewer',
    ...
    {
      /**
       * Идентичность зрителя без учётной записи.
       *
       * @remarks
       * Служит тем же, чем учётная запись у вошедшего: по нему находится право.
       * Выдаётся сервером при первой встрече и лежит в токене зрителя.
       *
       * Нужно затем, чтобы право по коду существовало записью и у того, кто
       * учётной записи не заводил: пока оно жило в токене, снять его было
       * нельзя - сервер о нём не знал.
       *
       * Личности за ним нет: это не имя человека, а имя его доступа.
       */
      name: 'ref',
      label: 'Идентичность',
      type: 'text',
      index: true,
      admin: {
        description: 'Метка браузера, которому выдан доступ. Пусто, если доступ на учётную запись.',
        readOnly: true,
      },
    },
    {
