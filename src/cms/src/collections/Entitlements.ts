 ...
 * плейлиста. Так платный набор может содержать открытое начало: оно доступно
 * само по себе, а закрытое открывает право на набор.
 *
...
export const Entitlements: CollectionConfig = {
  ...
  access: {
    ...
    // Иначе список покупателей чужого набора читался бы кем угодно.
    read: ({ req: { user } }) => {
