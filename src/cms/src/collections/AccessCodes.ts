...
/**
 * AccessCodes — коды и ссылки, открывающие набор.
 *
 * @remarks
 * Способ выдачи права, а не отдельный вид доступа: код срабатывает и
 * превращается в обычное право «зритель × набор». Когда придёт платёжка,
 * она встанет рядом и будет создавать такие же права.
 *
 * Ключевая настройка — нужен ли вход. Платный курс выдаётся с входом: иначе
 * доступ нельзя ни отозвать адресно, ни посчитать, а разошедшийся по чатам
 * код открывает курс всем, кто его увидел. Бесплатный промо-доступ уместно
...
export const AccessCodes: CollectionConfig = {
  ...
  admin: {
    ...
    group: 'Видео',
    description: 'Открывают набор по коду или ссылке. Превращаются в обычное право доступа.',
  },
  fields: [
    {
      name: 'code',
    ...
    },
    {
      ...
      admin: {
        description:
          'С входом доступ закрепляется за человеком: его видно в списке и можно отозвать. Без входа доступ живёт в браузере — отозвать адресно нельзя, а код, разойдясь по чатам, откроет набор всем.',
      },
    },
    {
      name: 'maxUses',
    ...
    },
    {
       ...
       * полученный по нему доступ — неделю. Так устроены подарки «посмотреть
       * до конца недели».
       */
      name: 'grantDays',
  ...
  hooks: {
    beforeValidate: [
      ({ data }) => {
        ...
        // его адресно нельзя, и без срока с лимитом он открывает набор
