...
/** Имя витрины движка — только для этого пресета, инстансам не уезжает. */
const WHG_SITE_NAME = 'Web Holy Grail';

/**
 * createLandingPage — собирает лендинг самого Web Holy Grail как Payload Page.
 *
 * @remarks
 * Гоняется **только на whg.sawking.tech** — это витрина движка. Новому сайту
 * она не нужна: `seed:minimal` ставит нейтральную пустую главную (#72).
 *
 * Идеология: эта страница — не статика в JSX, а **обычный Payload
...
export async function createLandingPage(
  ...
  const siteNameIsUntouched =
    !existingSettings.siteName ||
    existingSettings.siteName === DEFAULT_SITE_NAME ||
    existingSettings.siteName === WHG_SITE_NAME;
  if (siteNameIsUntouched) {
    await payload.updateGlobal({
      slug: 'site-settings',
      data: {
        siteName: WHG_SITE_NAME,
        logo: media.logo.id,
        contacts: {
  ...
  if (existing.docs.length > 0) {
    ...
    const updated = await payload.update({
      collection: 'pages',
      id: existingPage.id,
      data: buildLandingPageData(media),
    });
 ...
 */
function buildLandingPageData(media: MediaMap) {
  return {
    title: 'Главная',
    slug: 'home',
    ...
    blocks: [
      ...
      // 4. Comparison — cycle of pain (failure + повторная стоимость) vs WHG growth.
      {
        ...
        rightItems: [
          ...
          { text: 'Новые сервисы растут поверх существующих' },
        ],
      },
