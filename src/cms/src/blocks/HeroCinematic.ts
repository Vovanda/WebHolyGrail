...
export const HeroCinematicBlock: Block = {
  ...
  fields: [
    {
      name: 'video',
      label: 'Видео из медиатеки',
      type: 'upload',
      relationTo: 'media',
      filterOptions: () => ({ mimeType: { like: 'video' } }),
      admin: {
        description:
          'Загруженное видео из раздела «Медиа». Нарезка готовится сама после загрузки: обложка берёт её кусками, поэтому кадр появляется сразу и каждый круг начинается с нового места. Длина и вес значения не имеют.',
      },
    },
    {
      name: 'videoUrl',
      ...
      admin: {
        description:
          'Прямая ссылка на файл — на случай, когда видео лежит не в медиатеке. Файл тянется целиком, поэтому держите его коротким и лёгким: 5-10 секунд и до 5 МБ. Видео из медиатеки, если оно выбрано, показывается вместо этой ссылки.',
