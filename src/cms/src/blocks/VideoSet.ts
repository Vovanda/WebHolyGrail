...
export const VideoSetBlock: Block = {
  ...
  fields: [
    ...
    {
      /**
       * Как показывать список плейлиста.
       *
       * @remarks
       * Виды взаимоисключающие: колонка стоит рядом с плеером, лента лежит под
       * ним, панель прячет список за кнопкой в углу. При панели под плеером не
       * рисуется ничего - в этом и смысл, освободить место.
       */
      name: 'listView',
      label: 'Как показывать список',
      type: 'select',
      defaultValue: 'column',
      admin: {
        condition: (_data, sibling) => sibling?.['mode'] !== 'list',
        description: 'Панель прячет список за кнопкой над плеером и освобождает место.',
      },
      options: [
        { label: 'Колонкой рядом с плеером', value: 'column' },
        { label: 'Лентой под плеером', value: 'row' },
        { label: 'Панелью за кнопкой', value: 'panel' },
      ],
    },
    {
      /**
       * Переключатель вида для посетителя.
       *
       * @remarks
       * Витринная вещь: на демо шаблона он показывает, что вид настраивается, и
       * заменяет посетителю поход в админку. На обычном сайте вид выбирает
       * владелец, и переключать его посетителю незачем.
       */
      name: 'showViewSwitch',
      label: 'Дать посетителю переключать вид',
      type: 'checkbox',
      defaultValue: false,
      admin: {
        condition: (_data, sibling) => sibling?.['mode'] !== 'list',
        description: 'Нужно витрине шаблона. На обычном сайте оставьте выключенным.',
      },
    },
    {
      name: 'showCover',
