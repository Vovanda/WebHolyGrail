...
import type { SocialPostMedia } from 'contracts';
...
export function SocialMediaGrid({
  ...
  // Album photos — cap до MAX_GRID_ITEMS (6). На последней карточке overlay «+N»
  // если media.length больше: пользователь видит счётчик скрытых, клик открывает
  // lightbox с полным набором. originalIndex сохраняет связь с lightbox-slide.
  const photos = useMemo<PhotoExt[]>(() => {
    const shown = media.slice(0, MAX_GRID_ITEMS);
    const extra = Math.max(0, media.length - MAX_GRID_ITEMS);
    return shown.map((m, i): PhotoExt => {
      ...
      const base: PhotoExt = {
        ...
        originalIndex: i,
        extraCount: i === shown.length - 1 && extra > 0 ? extra : 0,
      };
      if (m.duration !== undefined) base.duration = m.duration;
      if (m.title !== undefined) base.title = m.title;
      return base;
    });
  }, [media]);

  // Lightbox slides — ВСЕ media (без cap), чтобы по клику на «+N» юзер пролистал
  // скрытые. Видео маркируется приватным флагом `__video`, render.slide ниже
  // перехватывает на iframe-embed. Стандартный type='image' для срабатывания
  // дефолтного pipeline lightbox'а (он гарантированно вызывает render.slide).
        ...
        <RowsPhotoAlbum
          ...
          render={{
            extras: (_, { photo }) => {
              ...
              return (
                <>
                  {p.isVideo && <VideoOverlay duration={p.duration} title={p.title} />}
                  {p.extraCount ? <PlusMoreOverlay count={p.extraCount} /> : null}
                </>
              );
            },
          }}
      ...
      <Lightbox
        ...
        render={{
          slide: ({ slide }) => {
            ...
            if (s.__video && s.__embedSrc) {
              const embedUrl = s.__embedSrc.includes('?')
                ? `${s.__embedSrc}&autoplay=1`
                : `${s.__embedSrc}?autoplay=1`;
              return (
                <div className="w-full h-full flex items-center justify-center p-4">
                  <iframe
                    src={embedUrl}
                    title={s.__title ?? 'Видео'}
...
function PlusMoreOverlay({ count }: { count: number }) {
  return (
    <span
      aria-hidden
      className={cn(
        ...
        'font-display text-3xl md:text-4xl font-semibold tracking-wide',
      )}
    >
      +{count}
    </span>
