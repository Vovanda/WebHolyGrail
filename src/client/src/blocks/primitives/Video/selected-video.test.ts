...
describe('выбор по адресу', () => {
  ...
  it('код закрытого видео выбирает именно его: там стоит форма ввода', () => {
    // Прежде здесь ждали подмены на играющее - тогда на месте закрытого был
    // пустой кадр, и уводить от него было правильно. Теперь там форма ввода
    // кода, и человек, пришедший по ссылке на закрытое, должен попасть к ней.
    expect(selectVideo(items, 'aaa')?.code).toBe('aaa');
  });
