 ...
 * Число наружу не выходит - ответ тот же, что у неверного кода.
 */
const CODE_LENGTH = 6;
...
export function AccessCodeForm({ className }: AccessCodeFormProps) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    /*
      Короткий ввод до сервера не доходит и попытку из лимита не тратит. Текст
      при этом тот же, что у неверного кода: сказать «не хватает знаков» значит
      назвать длину кода тому, кто их перебирает.
    */
    if (code.replace(/[^0-9A-Za-z]/g, '').length < CODE_LENGTH) {
      setError(REASON['invalid']!);
      return;
    }
