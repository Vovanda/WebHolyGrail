import type { PedigreeAncestor } from 'contracts';
...
function PCard({
  ...
  // Резолвер ссылок: если предок — наша Dog (`ourSlug` есть), → /dog/<slug>
  // + `data-detail-dialog="dog:<slug>"` (наша модалка). Иначе если есть rkfId
  // — РКФ-каталог + `data-detail-dialog="rkf:<NAME>"` (каскадная модалка).
  const ourSlug = ancestor.ourSlug;
  const href = ourSlug
    ? `/dog/${ourSlug}`
    : ancestor.rkfId
      ? hrefForAncestor(ancestor.rkfId)
      : null;
  const dataDialog = ourSlug
    ? `dog:${ourSlug}`
    : ancestor.rkfId
      ? `rkf:${ancestor.name.toUpperCase().replace(/Ё/g, 'Е')}`
      : null;
  const Tag = href ? 'a' : 'div';
  return (
    <Tag
      {...(href ? { href } : {})}
      {...(dataDialog ? { 'data-detail-dialog': dataDialog } : {})}
      className={cn(
