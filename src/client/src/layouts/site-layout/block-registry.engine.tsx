...
/**
 * Реестр блоков движка - маппинг имени блока на компонент.
 *
 * @remarks
 * Набор ездит обновлением целиком и всегда полон: новый общий блок доезжает
 * до сайтов сам. Доменные блоки сайта дописываются в точке сборки рядом -
 * `block-registry.tsx`, которую обновление не трогает.
 *
 * Раньше реестр ездил зеркалом целиком, и регистрация доменного блока сайта
 * стиралась каждым обновлением (R10): в описании это даже было узаконено
 * словами «строку регистрации придётся вернуть».
 */
type BlockRenderer = (node: BlockNode, settings: SiteSettings, children?: ReactNode) => ReactNode;

export const engineRegistry: Record<string, BlockRenderer> = {
...
export function renderBlockNode(
  ...
  children?: ReactNode,
  registry: Record<string, BlockRenderer> = engineRegistry,
): ReactNode {
