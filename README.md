...
### Видео живёт на самом сайте

Записи лежат в вашем хранилище и играют без сторонних площадок. Загруженный файл
режется на короткие куски сразу в нескольких качествах, куски шифруются, обложка
берётся кадром из середины. Закрыть или открыть запись можно в любую секунду:
меняется только то, кому сервер выдаёт ключ, а перенарезки не требуется.

Плеер умеет то, к чему зритель привык: субтитры несколькими языками, оглавление
главами, кадры-подсказки при перемотке, ссылку с таймкодом, продолжение с места
остановки и уголок при прокрутке страницы. Доступ к закрытому даётся кодом,
который владелец выдаёт сам.

Подробно — [`docs/whg/39-video.md`](docs/whg/39-video.md).
