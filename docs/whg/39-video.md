...
## Почему это дёшево раздаётся

Сегменты шифруются один раз и одинаковы для всех, кто их запрашивает. Отсюда следствие, ради
которого схема и выбрана: **раздача полностью ложится на кеш CDN**.

- Узел сети кладёт кусок один раз и отдаёт его всем зрителям. Origin отдаёт каждый сегмент
  однократно, а не по разу на человека.
- **Закрытое видео тоже кешируется.** Без ключа сегмент — набор случайных байт, поэтому платный
  материал спокойно лежит на чужих узлах по всей стране, и это не утечка.
- Переключение открытого в закрытое кеш не сбрасывает: меняется политика выдачи ключа, а байты
  прежние. Прогревать заново нечего.
- Перезалитый ролик получает новый адрес нарезки, поэтому со старым кешем не сталкивается.

Мимо кеша идёт единственный запрос — конверт с ключом. Он персональный, помечен как некешируемый,
и его размер измеряется десятками байт.

Сравнение с привычным подходом «отдавать закрытое по подписанным ссылкам»: там адрес уникален для
каждого зрителя, поэтому кеш либо не попадает вовсе, либо забивается копиями одного и того же.
Плата — скорость и оплаченный трафик origin.

> **Отсюда развилка.** Короткоживущие подписанные ссылки на сегменты (см. защиту от слива) этот
> выигрыш отменяют. Включать их имеет смысл там, где утечка дороже трафика, либо перекладывать
> проверку на сам CDN, чтобы узел решал доступ сам и кеш оставался общим.
