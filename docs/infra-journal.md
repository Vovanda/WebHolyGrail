...
| Секреты — защита #1 | `.gitignore` (`.env` запрет, `secrets/` запрет)                        | 2026-06-19 (8017d36)          | `.gitignore`                                                     | Первая линия — физически не дать `git add .env`                  |
...
| CMS                 | Payload 3.85 + Next 15 + React 19 + SQLite (drizzle)                   | 2026-06-19 (этот коммит)      | `sites/veo55/src/cms/`                                           | Контент-слой, админка на русском, JWT-auth, SQLite-файл в volume |
