...
| Local docker stack  | Docker Compose (client + cms, named volumes, bridge net)               | 2026-06-19 (этот коммит)      | `sites/veo55/deploy/local/`                                      | Воспроизводимый локальный стек; healthcheck'и + depends_on service_healthy; SQLite/Media в named volumes для бэкапа                                      |
