...
function copyFile(src, dst, label, mirror = true) {
  ...
  /*
    ...
    Совпавшее содержимое столкновением не считается: файл уже такой, каким его
    положил бы синк, подменять нечего. Так бывает, когда правку перенесли руками
    до первого обновления - прежде прогон вставал на ней стеной и требовал
    переименовать файл, идентичный шаблонному.
  */
  if (mirror && exists && !previous.has(rel) && !firstMeeting && !sameContent(src, dst)) {
    if (!force) {
      conflicts.push(rel);
      return;
    }
    ...
    */
    ...
    for (const поле of ['dependencies', 'devDependencies']) {
      ...
      for (const [имя, версия] of Object.entries(их)) {
        /*
          Внутренние пакеты не возим: у сайта они зовутся по-своему. Привезённая
          зависимость с именем из шаблона просто не находится в его рабочем
          пространстве, и установка падает целиком.
        */
        if (String(версия).startsWith('workspace:')) continue;
        if (!(имя in наши)) {
          наши[имя] = версия;
