...
/**
 * Следы работы: их нет в истории, но обход читает диск, а не историю.
 *
 * @remarks
 * Временная база после форматирования, отчёт прогона, снимок сборки - всё это
 * лежит в рабочей копии и в зеркало не попадает по смыслу, а попадало по факту:
 * синк с локального пути уносил их новому сайту, и тот начинал жизнь с чужим
 * мусором.
 */
const LEFTOVERS =
  /(\.(db|db-shm|db-wal|log|tsbuildinfo)$)|(\.(bak|orig|rej)($|[-.]))|((^|\/)\.tmp-)/;
...
const RULES = [
  ...
  */
  ...
  /*
    Фирменные цвета сайта. Подключаются в его макете после общих стилей и
    перебивают значения шаблона, не заменяя их.

    Правило заведено по потере: палитра питомника жила прямо в общем
    `styles/tokens.css`, а тот приезжает обновлением целиком - и 28 августа
    сайт стал синим вместо тёплого. Общий файл описывает palette по умолчанию,
    фирменное живёт здесь и обновлением не трогается.
  */
  [/^src\/client\/src\/app\/brand\.css$/, 'skip'],
  ...
  */
  ...
  /*
    Миграция, помеченная `.sync-skip` в имени - `<дата>_<имя>.sync-skip.ts`
    рядом с таким же `.json`, - остаётся у себя. Так помечается всё, что чужому
    сайту не нужно: таблицы блоков из `domain`, разовая починка данных, схема
    собственной коллекции. Без пометки сайт получал чужие пустые таблицы, и
    сверка схемы после каждого обновления объявляла расхождение там, где всё
    на месте.

    Маркер называет действие, а не причину: поводов не возить миграцию больше
    одного, и по имени видно решение, а не догадка о нём.

    Имя, а не папка: папку пришлось бы учить читать сборку списка миграций,
    сверку схемы и принятие истории, а имя не трогает ни одно из них - файл
    лежит там же, и порядок по дате сохраняется.
  */
  [/^src\/cms\/migrations\/.*\.sync-skip\.(ts|json)$/, 'skip'],
...
/**
 * Пометка в самом файле: `@sync-skip` в первых строках.
 *
 * @remarks
 * Второй способ сказать «этот файл остаётся у себя», рядом с суффиксом в имени.
 * Нужен там, где имя менять нельзя: имя применённой миграции записано
 * в `payload_migrations` каждой базы, и переименование заставит прогнать её
 * заново.
 *
 * Читаются только первые строки: пометка ставится в шапке, а перебирать
 * мегабайты схемы ради строки в конце незачем.
 */
export const SKIP_MARK = '@sync-skip';

/**
 * Как выглядит пометка в файле. Ставится в самой шапке, до импортов.
 *
 * @remarks
 * Рамка не для красоты: файл со схемой на тысячу строк листают глазами, и
 * строчка-комментарий в ней теряется. Пометка меняет судьбу файла - её видно
 * с первого экрана.
 */
export const SKIP_BANNER = [
  '/* ***************************************************************',
  ` *  ${SKIP_MARK} - файл остаётся у себя, синк не возит его по сайтам.`,
  ' *',
  ' *  Причина: <одной строкой, почему чужому сайту это не нужно>',
  ' *************************************************************** */',
].join(String.fromCharCode(10));

export function markedSkip(full) {
  /*
    Снимок схемы наследует судьбу своей миграции: пометку в него не вписать -
    это JSON, комментариев в нём нет, - а уехать без неё он не должен. Иначе
    у сайта остаётся снимок, описывающий чужие таблицы.
  */
  const target = full.endsWith('.json') ? full.slice(0, -'.json'.length) + '.ts' : full;
  try {
    const head = fs.readFileSync(target, 'utf8').slice(0, 2000);
    return head.includes(SKIP_MARK);
  } catch {
    return false;
  }
}
...
export function walk(root, base = root, out = []) {
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    if (LEFTOVERS.test(rel)) continue;
    out.push(rel);
  }
  return out;
}
