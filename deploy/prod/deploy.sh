...
# Выкладки разных сайтов идут по очереди.
#
# Схлопывание прогонов у сайта своё и работает как задумано: катится последний
# коммит, очереди внутри сайта нет. Но группа эта живёт внутри репозитория, а
# репозиторий у каждого сайта свой - и на общий хост четыре выкладки приходят
# разом. Образы у них собраны из одних слоёв, и containerd рвался на распаковке:
# «failed to extract layer … no such file or directory», а сайт оставался на
# прежней версии. Поймано 31.08.2026 на sng74 и veo55.
#
# Замок один на хост и берётся на всю выкладку: сосед ждёт, а не лезет в те же
# слои. Ожидание - минуты; на «десять коммитов подряд, через пять минут свежее
# на проде» это не влияет, потому что внутри сайта лишние прогоны по-прежнему
# отменяются, а до замка доходит только последний.
#
# Ждём долго и намеренно: упереться в занятый замок и выйти значило бы то же
# самое, что упасть на распаковке.
DEPLOY_LOCK="/tmp/whg-deploy.lock"
exec 9>"$DEPLOY_LOCK"
if ! flock -w 1800 9; then
  echo "✗ Полчаса ждали своей очереди на выкладку - на хосте что-то застряло." >&2
  exit 1
fi
echo "   ✓ очередь выкладки свободна"
...
echo
echo "→ Post-deploy cleanup (свои образы, свои контейнеры, общий кэш)..."
...
#
# Убираем по владению, а не по возрасту. Хост общий: пока один сайт убирает за
# собой, соседний создаёт свои контейнеры и тянет образы. Уборка «всё, что
# старше двух часов» сносила их прямо из-под него - выкладка падала на «нет
# такого контейнера», а сайт оставался на прежней версии. Возраст тут вообще не
# та мера: образ соседа может быть недельным и всё равно рабочим.
#
# Своё узнаётся по имени: образы `<реестр>/<владелец>/<слаг>-{cms,client}`,
# контейнеры `<слаг>-*`. Чужое не трогаем ни при каком возрасте.

# Образы, которые держат живые контейнеры - и свои, и чужие. Сравнение идёт по
# полному отпечатку: `docker ps` показывает то тег, то идентификатор, и по имени
# такое сравнение промахивается.
#
# Имена переменных здесь латиницей не по привычке: оболочка на прод-хосте
# кириллические имена не принимает и падает на «not a valid identifier», унося
# с собой прогон выкладки.
busy_images=$(docker ps -aq | xargs -r docker inspect --format '{{.Image}}' 2>/dev/null | sort -u || true)

docker images --no-trunc --format '{{.Repository}}:{{.Tag}} {{.ID}}' |
  grep -E "/${SITE_SLUG}-(cms|client):" |
  while read -r ref digest; do
    if echo "$busy_images" | grep -qxF "$digest"; then continue; fi
    # Образ этой самой выкладки не трогаем, даже если контейнер ещё не поднят:
    # вторая выкладка того же сайта успевает собрать его, пока первая убирает.
    case "$ref" in *:"$TAG") continue ;; esac
    docker rmi "$ref" >/dev/null 2>&1 && echo "   убран свой образ: $ref" || true
  done

# Свои остановленные контейнеры прошлых цветов. Живой цвет сюда не попадает: он
# в состоянии running.
docker ps -a --filter "name=^${SITE_SLUG}-" --filter "status=exited" -q |
  xargs -r docker rm >/dev/null 2>&1 || true

# Безымянные слои и кэш сборки ничьи - вот их и чистим по возрасту.
docker image prune -f --filter "until=2h" 2>&1 | grep -E "Total reclaimed" | tail -1 | sed 's/^/   /' || true
