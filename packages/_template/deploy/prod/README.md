...
VPS_HOST=root@veo55.ru ./sites/veo55/deploy/prod/deploy.sh
```
...
```bash
cp sites/veo55/deploy/prod/.env.production.example \
   sites/veo55/deploy/prod/.env.production
nano sites/veo55/deploy/prod/.env.production   # заполнить все
```
...
docker compose -f sites/veo55/deploy/prod/docker-compose.yml up -d nginx
...
docker compose -f sites/veo55/deploy/prod/docker-compose.yml restart nginx
```
...
infisical run --env=prod -- docker compose \
  -f sites/veo55/deploy/prod/docker-compose.yml up -d --build

# Вариант B — через .env.production файл:
docker compose --env-file sites/veo55/deploy/prod/.env.production \
  -f sites/veo55/deploy/prod/docker-compose.yml up -d --build
...
docker compose -f sites/veo55/deploy/prod/docker-compose.yml restart cms
docker exec veo55-cms pnpm --filter veo55-cms migrate
```
