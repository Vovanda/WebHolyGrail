...
pnpm compose:up
# что то же самое:
infisical run --env=dev -- docker compose -f sites/veo55/deploy/local/docker-compose.yml up -d
```
...
Появится в `sites/veo55/deploy/prod/` на Шаге 7 — там добавится nginx-reverse-proxy, TLS через Traefik / Caddy, и compose без `restart: unless-stopped` (а через systemd-unit на VPS).
