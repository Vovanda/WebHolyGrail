...
- Делаешь модалку-карточку любой сущности сайта (item / помёт / FAQ / член команды / товар / мастер / автомобиль — что угодно)
...
- **Не nav-drawer** (постоянная панель layout'а — NavDrawer, sidebar). Это `whg-layouts → Drawer-канон`.
...
- Симметрия с burger menu сайта (которое стоит top-right по умолчанию — см. `whg-layouts → Дефолт burger справа`)
...
Пример: модалка «карточка мастера автосервиса» для другого Holy Grail-сайта. Аналогично для item угодно.
...
- `sites/<slug>/.../DogDetailDrawer.tsx` + `sites/<slug>/.../app/internal/dog/[slug]/route.ts` — карточка item (right drawer на ПК, fullscreen на мобилке). Триггер — клик по чипу-кличке в постах /news.
...
- **R15** в `whg-rules` — client-fetch только same-origin через `/internal/*`. Это фундамент работы модалок через demo-tunnel и прод-домены.
...
- **`whg-layouts → Drawer-канон`** — про **навигационные** drawer'ы (NavDrawer, sidebar). Не путать с detail-модалками этого скилла.
