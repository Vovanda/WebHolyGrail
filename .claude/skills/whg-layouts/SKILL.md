...
## Режим боковой панели выбирает блок, а не лейаут

`SiteLayout` боковые панели не позиционирует: он рендерит их содержимое, а
`fixed`, ширину и анимацию задаёт сам блок. Поэтому в одном слоте живёт что угодно:

- **overlay** — выезжает поверх контента (так сделан NavDrawer);
- **сдвигающая** — открываясь, отодвигает центральную колонку. Стейт остаётся
  внутри блока, а сдвиг делается атрибутом на `body` и правилом в стилях: тянуть
  состояние наружу через глобальный стор не нужно;
- **постоянная колонка** — панель всегда на месте, центр сжат под неё.
