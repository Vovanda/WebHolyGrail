...
### Эталон `push` - как задуман лейаут

Всё решает одно: есть ли сбоку от средней секции свободное поле. Устройств два,
случаев привычно три.

**Поле есть.** Закрытая панель спрятана под листом целиком и на открытии выходит
из-под его края. Страница уступает `max(0, ширина панели − поле сбоку)`:

- поля хватает - уступка сама собой ноль, страница стоит: упереться панели
  не во что, толкать нечем;
- поля мало - панель упирается в край окна, страница отдаёт разницу.

**Поля нет.** Секция занимает окно целиком, прятать некуда: закрытая панель ждёт
за краем окна, и они со страницей едут одним листом справа налево, на всю ширину
панели.

Наполовину панель не прячут - хвост торчит в поле и виден. Порог между
устройствами - ширина самой средней секции, а не выбранная точка экрана.

Разбор с причинами - `docs/ux/side-panels.md`, реализация - `.side-panel--push`
в `globals.css`.
