---
name: whg-rules
description: Архитектурные правила Web Holy Grail из docs/whg/30-philosophy.md. R0 — контент в БД, не в коде. R1 — Tailwind прячется внутри компонентов. R2 — только токены, никаких bg-[#hex]. R3 — фронт ↔ CMS только через contracts/. R4 — масштабирование сбоку (новый модуль, не трогать ядро). R5 — блок = чистая функция от пропсов, имя функциональное, тема через токены. R6 — без догм по стилизации. R7 — нулевая нагрузка авансом. R8 — доступ к данным через слой, не напрямую в БД. R9 — обобщение снизу-вверх. R10 — шаблон не ломает собранное на нём. R11 — одна вещь делает одно дело: обработчик переводит HTTP в вызов, дело живёт в lib/ и проверяется без сети. R13 — ЧПУ для сущностей, query для представления. R14 — SSR по умолчанию. R15 — client-fetch только same-origin. Триггерить при любом архитектурном выборе: новый блок, эндпоинт, сценарий в lib/, contracts, коллекция, стилизация, client/server-разделение, обобщение.
---
...
- Создаёшь компонент или примитив → навык `whg-components`
- Пишешь эндпоинт, сценарий или модуль в `lib/` → R11
...
**Оформление описывается один раз и переиспользуется.** Любое оформительское
значение - цвет, тень, отступ, радиус, вид кнопки, вид карточки - живёт в общем
месте (`tokens.css`, `globals.css`) и берётся оттуда по имени. На месте оно
не сочиняется: собранное руками разъезжается с остальными в первый же месяц,
а палитру задаёт владелец - подмешанный чёрный на смуглой палитре читается грязью.

Перед тем как задать вид: `grep` по общему месту. Нужного нет - он **добавляется
туда**, а не собирается на месте.

Примеры того же правила:

- `shadow-[0_24px_64px_rgba(43,34,26,0.28)]` → токен `--shadow-overlay`
- `hover:bg-[#d1c69f]` → `hover:bg-surface-hover`
- `h-11 w-11 rounded-lg bg-surface border ...` вместо готового `action-button`
- `rgba()` в `style={{}}` и в обработчиках наведения
...
## R10. Шаблон не ломает то, что уже собрано на нём

На шаблоне живут пять сайтов. Синк зеркалит `blocks/primitives/`, `layouts/`, `ui/` —
**удалённый файл он уносит из каждого сайта**, и там встаёт сборка на первом же
импорте. Забранное свойство блока хоронит вид, который владелец собрал руками
в админке: галка исчезла — страница выглядит иначе, и вернуть нечем.

Порядок при выводе вещи из обращения:

1. Пометить `@deprecated` с указанием замены — и в самом файле, и в `index.ts`.
2. Перевести сайты (синк + правка их кода), убедиться, что ни один не ссылается.
3. Только потом удалить из шаблона.

Новая раскладка / новый вид приходит **свойством рядом со старым**, не вместо него.

❌ Удалить примитив, потому что «его никто не зовёт» — проверено по шаблону, а зовут
из сайтов. ❌ Заменить поведение блока на «лучшее» без галки на прежнее.

## R11. Одна вещь делает одно дело

Правило про компоненты — «чужая забота не течёт в соседа» — действует и на сервере,
где его нарушают чаще: у обработчика нет вёрстки, по которой видно, что он разросся.

**Мера — одна причина меняться.** Модуль, который правят и когда меняется способ
хранения, и когда меняется правило выдачи, — это два модуля, слипшихся в один.

**Обработчик эндпоинта переводит HTTP в вызов и обратно.** Разобрать запрос, позвать
дело, свернуть ответ. Само дело живёт в `lib/` и проверяется без сети и базы — так
устроены `redeem`, `grant-access`, `entitlements`: чистая функция сверху, Payload сбоку.

**Признаки, что вещь делает два дела:**

- имя честно описывается только через «и» — «погашает код **и** пишет право»;
- у функции есть довод, меняющий смысл её работы, а не данные;
- в обработчике идут подряд проверка частоты, чтение из базы, решение по правилу,
  запись и сборка ответа — каждый абзац отделён своим комментарием, потому что иначе
  их не различить;
- тест на одну заботу приходится начинать с поднятия трёх других.

❌ Обработчик на сто строк, где решение о доступе принимается между двумя запросами
к базе. ❌ Функция с флагом `dryRun`, `silent`, `alsoNotify`. ❌ Модуль `helpers.ts`.

Правило не про длину: сто строк одного дела — нормально, тридцать строк трёх дел — нет.
