...
Таблица ровная, но веса у навыков разные. Три работают почти всегда:
`whg-session-start` открывает сессию, `whg-plan` ведёт работу длиннее одного
действия, **`whg-components` включается перед каждым компонентом и примитивом** -
а из них состоит весь шаблон, и именно на них мы обжигались чаще всего. Остальные
поднимаются, когда попал в их зону.
